...
from bot_message_func import send_bot_message
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        await context.bot.delete_message(chat_id=user_id, message_id=chat_data.message_id)
    except:
        pass
    if str(user_id) in os.getenv("ALLOWED_USERS"):
        ...
        await send_bot_message(update, context, message_text=text, reply_markup=reply_markup)
    else:
        title = main_menu.no_access
        text = f"*{title}*" + mod_fix()
...
async def restart(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if str(user_id) not in os.getenv("ALLOWED_USERS"):
        await context.bot.send_message(chat_id=user_id, text=main_menu.no_access)
    else:
